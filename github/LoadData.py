...
class LoadDiaData(Dataset):
    def __init__(self, train_or_test):
        ...
        file=open('./target/10/MSP_feature.pkl','rb')
        _,self.videoIDs, self.videoSpeakers, _,self.videoLabels,_,self.videoText,_ ,self.videoAudio, \
        self.videoSentence,_,_,self.trainVid, self.testVid= pickle.load(file, encoding='latin1')
    ...
    def __getitem__(self, batch_index):
        ...
        audio_len = len(self.videoAudio[vid])  #一个sesion中的话语数量
        gap = self.text_audio_max - audio_len  #gap是为了让每个session中的话语数量相同
        ...
        mask[:text[1]] = 1   #让非填充的mask置为1，其他的置为0，用于掩盖掉pad的多余东西
        labels=functional.one_hot(labels, num_classes=label_classes).squeeze(1)
        return  audio[0].type(torch.FloatTensor), text[0].type(torch.FloatTensor), mask, labels, text[1]
